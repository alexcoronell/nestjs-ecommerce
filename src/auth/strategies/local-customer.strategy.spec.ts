...
describe('LocalCustomerStrategy', () => {
  ...
  it('should return user if validation succeeds', async () => {
    ...
    mockAuthService.validateCustomer.mockResolvedValue(user);

    await expect(strategy.validate('test', 'pass')).resolves.toEqual(user);
    expect(mockAuthService.validateCustomer).toHaveBeenCalledWith(
      'test',
      'pass',
    );
  });
  ...
  it('should throw UnauthorizedException if validation fails', async () => {
    mockAuthService.validateCustomer.mockResolvedValue(null);
